...
\item Guia do Utilizador - Tudo que o utilizado precisa saber para usar o BlackArch de forma efetiva
...
BlackArch é uma distribuição Linux para testes de penetração e pesquisadores de segurança.É uma derivação do \href{https://www.archlinux.org/}{ArchLinux} e os utilizadores podem instalar componentes do BlackArch individualmente ou por grupos em cima da distribuição.
...
{unofficial user repository} então  pode instalar o BlackArch em cima de uma instalação Arch Linux já existente.Os pacotes pode ser instalados individualmente ou por categorias.
...
\section{Junte-se ao BlackArch}
 pode se comunicar com a equipe BlackArch através das formas abaixo:
...
O BlackArch é compatível como toda instalação do Arch. A instalação é feita através de um repositório não oficial, se  quiser usar uma ISO olhe na seção \href{https://www.blackarch.org/downloads.html#iso}{Live ISO}.
...
\begin{lstlisting}
   ...
   sudo ./strap.sh
\end{lstlisting}
Agora faça download de uma copia atualizada da lista de pacotes e os sincronize.
\begin{lstlisting}
  sudo pacman -Syyu
\end{lstlisting}
...
Agora  pode instalar os pacotes do repositório do blackarch.
\begin{enumerate} 
...
De maneira alternativa  pode criar os pacotes do BlackArch diretamente do codigo fonte.Pode encontrar os PKGBUILDs no
\href{https://github.com/BlackArch/blackarch/tree/master/packages}{github}. Para criar todo o repositório  pode usar a ferramenta
\href{https://github.com/BlackArch/blackman}{Blackman}.
\begin{itemize}
\item Primeramente,  tem que instalar o Blackman. Se o repositório do BlackArch está configurado na sua máquina,  pode instalar o Blackman:
\begin{lstlisting}
  pacman -S blackman
\end{lstlisting}

\item  pode construir e instalar o Blackman pelo código fonte:
\begin{lstlisting}
  mkdir blackman
  cd blackman
...
\item Ou  pode instalar o Blackman pelo AUR::
\begin{lstlisting}
...
\subsection{Uso básico do Blackman} Blackman possui um uso simples, seus parâmetros são um pouco diferentes do que  esperaria de algo como o pacman. O básico será mostrado abaixo.
\begin{itemize}
\item Descarrega, compila e instala o pacote:
\begin{lstlisting}
  sudo blackman -i pacote
\end{lstlisting}

\item Descarrega, compila e instala toda uma categoria:
\begin{lstlisting}
  sudo blackman -g grupo
\end{lstlisting}

\item Descarrega, compila e instala todas as ferramentas do BlackArch:
\begin{lstlisting}
  sudo blackman -a
\end{lstlisting}
...
\subsection{Instalação pelo live-, netinstall- ISO ou ArchLinux}
 pode instalar o BlackArch através de um live- ou netinstall-ISOs.\\Veja em
...
\subsection{Grupos}
Permite que os utilizadores instalem um tipo específico de pacote de forma rápida e fácil, os pacotes são separados por grupos. Os grupos permitem que o utilizador digite "pacman -S <group name>" e instale vários pacotes de uma vez.

\subsubsection{blackarch}
O grupo blackarch é o grupo base onde todos os pacotes se encontram. Isto permite que o utilizador instale todos os pacotes de uma só vez com facilidade.
...
Pacotes para permitir que o utilizador veja o que um programa em particular esta "fazendo" em tempo real.
...
Pacotes para proteger o utilizador de malware \& ataques de outros utilizadores.
...
\section{Estrutura do repositório}
 pode encontrar o repositório git BlackArch principal aqui:
...
Essa seção mostra como  pode contribuir com o projeto BlackArch Linux.
...
Se melhor a se fazer se  trabalha remotamente é realizar o fork e o seu pull request para adicionar no repositório principal.
\begin{lstlisting}
...
está configurado corretamente.Isto não deve causar problemas ao menos que  tenhas
alguns direitos de commit com não habilitar o push sem o mesmo.

Se  tem a habilidade de commitar, é mais fácil usar o
\textit{git@github.com:blackarch/blackarch.git} mas ai é com .
